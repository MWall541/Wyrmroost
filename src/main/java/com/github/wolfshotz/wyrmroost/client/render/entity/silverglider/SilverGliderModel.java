...
public class SilverGliderModel extends WREntityModel<SilverGliderEntity>
{
    ...
    public SilverGliderModel()
    {
        ...
        mouthTop.setPivot(0.0F, -0.7F, -4.5F);
        ...
        phalangeR1.setPivot(-14.0F, 0.0F, -0.5F);
        ...
        wingMembraneElbowL.setPivot(-9.3F, -0.37F, -0.1F);
        ...
        phalangeSegmentL1.setPivot(-1.4F, 0.4F, -0.6F);
        ...
        phalangeSegmentL2.setPivot(-9.7F, 0.0F, 0.0F);
        ...
        legR3.setPivot(-0.02F, 2.5F, 0.1F);
        ...
        phalangeSegmentR1.setPivot(-1.4F, 0.4F, 0.5F);
        ...
        legL2.setPivot(0.38F, 3.1F, -0.1F);
        ...
        tail2.setPivot(-0.02F, -0.02F, 2.8F);
        ...
        wingMembraneR2.pitch = -0.0001f;
        wingMembraneR3.pitch = 0.0001f;
        wingMembraneL2.pitch = -0.0001f;
        wingMembraneL3.pitch = 0.0001f;
    ...
    @Override
    public void setAngles(SilverGliderEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch)
    {
        if (netHeadYaw < -180) netHeadYaw += 360;
        else if (netHeadYaw > 180) netHeadYaw -= 360;
        if (entity.flightTimer.get() == 1)
        {
            mainBody.roll = -(netHeadYaw * Mafs.PI / 180f) * 0.5f;
            mainBody.pitch = headPitch * Mafs.PI / 180f;

            float foldAmount = Math.max(headPitch * Mafs.PI / 180f, 0) * 0.75f;
            wing1R.yaw += foldAmount;
            wing2R.yaw += -foldAmount * 1.8;
            phalangeR1.yaw += foldAmount * 2;
            phalangeR2.yaw += foldAmount;
            phalangeR3.yaw += foldAmount;
            wingMembraneR2.pivotX += foldAmount * 4;
            wingMembraneR2.yaw += foldAmount;

            wing1L.yaw -= foldAmount;
            wing2L.yaw -= -foldAmount * 1.8;
            phalangeL1.yaw -= foldAmount * 2;
            phalangeL2.yaw -= foldAmount;
            phalangeL3.yaw -= foldAmount;
            wingMembraneL2.pivotX += foldAmount * 4;
            wingMembraneL2.yaw -= foldAmount;
        }
        else faceTarget(netHeadYaw, entity.isFlying()? 0 : headPitch, 1, headArray);
    }

    @Override
    public void animateModel(SilverGliderEntity entity, float limbSwing, float limbSwingAmount, float partialTick)
    {
        this.entity = entity;
        float frame = entity.age + partialTick;
        resetToDefaultPose();
