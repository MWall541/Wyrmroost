...
import WolfShotz.Wyrmroost.util.world.WRWorldType;
...
public class WyrmroostBiomeProvider extends BiomeProvider
{
    ...
    public WyrmroostBiomeProvider(WorldInfo info, WyrmroostChunkGenerator.Config settings)
    {
        Layer[] alayer = LayerUtil.buildOverworldProcedure(info.getSeed(), WRWorldType.WYRMROOST, settings);
    ...
    public Set<BlockState> getSurfaceBlocks()
    {
        if (topBlocksCache.isEmpty())
            Biome.BIOMES.forEach(b -> topBlocksCache.add(b.getSurfaceBuilderConfig().getTop()));
